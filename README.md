...
from another component and call its functions from that other component, but you cannot do that with functional component.
...
How useEffect works is, it uses a dependency array as its second parameter to determine when it should re-run the callback it
recieves as its first parameter. It calls the callback function returned by that function when the component is unmounted.
By using an empty dependency array, We tell the useEffect hook to only run this function once after the component mounts, like how
...
As a general rule, if the functional component starts looking too complicated, its usually easier to just transform it into a 
class component.
